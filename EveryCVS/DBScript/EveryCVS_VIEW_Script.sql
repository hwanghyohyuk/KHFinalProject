...
MIN(STORE_PRODUCT_NO)AS STORE_PRODUCT_NO, TRUNC(MIN(C.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24) AS MIN_EXPIRATION_HOUR, B.PRICE
...
GROUP BY USER_NO, A.BRAND_NO,  D.BRAND_NAME, A.STORE_NO, STORE_NAME, B.PRODUCT_NO, B.PRODUCT_NAME, B.STORED_FILE_NAME, B.PRICE
WITH READ ONLY;
...
SELECT A.*,B.BRAND_NAME,C.STORE_NAME FROM TB_USER A 
LEFT OUTER JOIN TB_BRAND B ON(B.BRAND_NO = A.BRAND_NO) 
LEFT OUTER JOIN TB_STORE C ON(A.STORE_NO = C.STORE_NO)  
ORDER BY 1 ASC;
