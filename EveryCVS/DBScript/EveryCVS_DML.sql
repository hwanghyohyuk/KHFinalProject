...
GROUP BY USER_NO, A.BRAND_NO,  D.BRAND_NAME, A.STORE_NO, STORE_NAME, B.PRODUCT_NO, B.PRODUCT_NAME, B.STORED_FILE_NAME,STORE_PRODUCT_NO;
--MIN(STORE_PRODUCT_NO)AS 
...
SELECT A.USER_NO, D.BRAND_NO, D.BRAND_NAME, B.STORE_NO, B.STORE_NAME, A.PRODUCT_NO, C.PRODUCT_NAME, C.STORED_FILE_NAME,
(SELECT STORE_PRODUCT_NO FROM TB_STORE_PRODUCT E WHERE A.STORE_NO = E.STORE_NO AND A.PRODUCT_NO = E.PRODUCT_NO)
FROM TB_FAVORITE_LIST A
JOIN TB_STORE B ON(A.STORE_NO = B.STORE_NO)
JOIN TB_PRODUCT C ON(C.PRODUCT_NO = A.PRODUCT_NO)
JOIN TB_BRAND D ON(B.BRAND_NO=D.BRAND_NO);

SELECT * FROM TB_STORE_PRODUCT;
...
(TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 AS EXPIRATION_RATE,
(CASE
    WHEN
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 <70 AND
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 >=60    
    THEN '10%'
   WHEN
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 <60 AND
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 >=50    
    THEN '20%'
       WHEN
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 <50 AND
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 >=40    
    THEN '30%'
       WHEN
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 <40 AND
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 >=30    
    THEN  '40%'
       WHEN
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 <30 AND
    (TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24,0)/(B.EXPIRATION_DATE*24))*100 >=0    
    THEN  '50%'
    ELSE '0%'
