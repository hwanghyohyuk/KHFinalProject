...
CREATE TABLE TB_SERVICE_CENTER (
	...
	ORIGINAL_FILE_NAME VARCHAR2(300), /* 첨부파일기존명 */
	RENAME_FILE_NAME VARCHAR2(300) /* 첨부파일변경명 */
);
...
COMMENT ON COLUMN TB_SERVICE_CENTER.RENAME_FILE_NAME IS '첨부파일변경명';
...
CREATE TABLE TB_USER (
	...
	ORIGINAL_PROFILE_NAME VARCHAR2(300), /* 프로필기존명 */
	RENAME_PROFILE_NAME VARCHAR2(300), /* 프로필변경명 */
...
COMMENT ON COLUMN TB_USER.ORIGINAL_PROFILE_NAME IS '프로필기존명';

COMMENT ON COLUMN TB_USER.RENAME_PROFILE_NAME IS '프로필변경명';
...
CREATE TABLE TB_PRODUCT (
	...
	ORIGINAL_IMAGE VARCHAR2(300), /* 기존이미지 */
	RENAME_IMAGE VARCHAR2(300), /* 변경이미지 */
...
COMMENT ON COLUMN TB_PRODUCT.ORIGINAL_IMAGE IS '기존이미지';

COMMENT ON COLUMN TB_PRODUCT.RENAME_IMAGE IS '변경이미지';
...
COMMENT ON COLUMN TB_EVENT.RENAME_FILE_NAME IS '첨부파일변경명';
...
COMMENT ON COLUMN TB_PURCHASE.STORE_NO IS '지점번호';
...
COMMENT ON COLUMN TB_PURCHASE.POINT IS '적립포인트';
...
COMMENT ON COLUMN TB_BOARD.RENAME_FILE_NAME IS '첨부파일변경명';
...
CREATE TABLE TB_PRODUCT_KIND (
	...
	PRODUCT_KIND_NAME <지정 되지 않음> /* 상품종류명 */
);
...
COMMENT ON COLUMN TB_EVENT_RESULT.RENAME_FILE_NAME IS '첨부파일변경명';
...
COMMENT ON COLUMN TB_SERVICE_CENTER_COMMENT.ORIGINAL_FILE_NAME IS '첨부파일기존명';

COMMENT ON COLUMN TB_SERVICE_CENTER_COMMENT.RENAME_FILE_NAME IS '첨부파일변경명';
...
ALTER TABLE TB_PRODUCT
	ADD
		CONSTRAINT FK_TB_PRDCT_KND_TO_TB_PRDCT
		FOREIGN KEY (
			PRODUCT_KIND_NO
		)
...
ALTER TABLE TB_PURCHASE
	ADD
		CONSTRAINT FK_TB_STORE_TO_TB_PURCHASE
		FOREIGN KEY (
			STORE_NO
		)
		REFERENCES TB_STORE (
