...
DROP VIEW STORE_MAP_VIEW;

DROP VIEW SALE_VIEW;

DROP VIEW SALE_PRODUCT_KIND_VIEW;

DROP VIEW SALE_PRODUCT_KIND_DATE_VIEW;
...
SELECT Y.USER_NO,D.BRAND_NO, F.BRAND_NAME, Y.STORE_NO, D.STORE_NAME, Y.PRODUCT_NO, E.PRODUCT_NAME,Y.STORE_PRODUCT_NO, E.STORED_FILE_NAME, Y.MIN_EXPIRATION_MINUTE
FROM(
SELECT X.USER_NO,X.STORE_NO,X.PRODUCT_NO
,MIN(X.STORE_PRODUCT_NO) KEEP (DENSE_RANK FIRST ORDER BY X.EXPIRATION_MINUTE) AS STORE_PRODUCT_NO
, MIN(X.EXPIRATION_MINUTE) AS MIN_EXPIRATION_MINUTE
FROM(
SELECT A.*
,C.STORE_PRODUCT_NO
,TRUNC((C.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24*60) AS EXPIRATION_MINUTE
FROM TB_FAVORITE_LIST A 
JOIN TB_PRODUCT B ON(B.PRODUCT_NO = A.PRODUCT_NO)
JOIN TB_STORE_PRODUCT C ON(A.STORE_NO = C.STORE_NO AND A.PRODUCT_NO = C.PRODUCT_NO)) X
WHERE EXPIRATION_MINUTE>0
GROUP BY X.USER_NO,X.STORE_NO,X.PRODUCT_NO) Y
JOIN TB_STORE D ON(D.STORE_NO = Y.STORE_NO)
JOIN TB_PRODUCT E ON(E.PRODUCT_NO = Y.PRODUCT_NO)
JOIN TB_BRAND F ON(F.BRAND_NO=D.BRAND_NO)
WITH READ ONLY;
...
A.CALCULATED_PRICE,A.USING_POINT, A.ACCUMULATE_POINT,A.PURCHASE_DATE, E.STORED_FILE_NAME
FROM TB_PURCHASE A
...
SELECT A.STORE_PRODUCT_NO, A.STORE_NO, A.PRODUCT_NO, B.PRODUCT_NAME, B.MANUFACTURER, B.PRICE, A.MANUFACTURE_DATE, B.EXPIRATION_DATE,
TRUNC((A.MANUFACTURE_DATE+B.EXPIRATION_DATE-SYSDATE)*24*60) AS EXPIRATION_MINUTE, A.QUANTITY, B.PRODUCT_KIND_NO, F.PRODUCT_KIND_NAME, B.BRAND_NO, H.BRAND_NAME,
...
/*종류별 판매량 날짜 추가*/
CREATE VIEW SALE_PRODUCT_KIND_DATE_VIEW AS
SELECT C.STORE_NO, C.BRAND_NO, D.PRODUCT_KIND_NO, E.PRODUCT_KIND_NAME,SUM(A.PURCHASE_QUANTITY) AS SALE_QUANTITY, TO_DATE(TO_CHAR(A.PURCHASE_DATE,'YY/MM/DD')) AS SALE_DATE
FROM TB_PURCHASE A 
JOIN TB_STORE_PRODUCT B ON(A.STORE_PRODUCT_NO = B.STORE_PRODUCT_NO)
JOIN TB_STORE C ON(B.STORE_NO = C.STORE_NO)
JOIN TB_PRODUCT D ON(B.PRODUCT_NO = D.PRODUCT_NO)
JOIN TB_PRODUCT_KIND E ON(D.PRODUCT_KIND_NO = E.PRODUCT_KIND_NO)
GROUP BY C.STORE_NO, C.BRAND_NO, D.PRODUCT_KIND_NO, E.PRODUCT_KIND_NAME, TO_DATE(TO_CHAR(A.PURCHASE_DATE,'YY/MM/DD'))
WITH READ ONLY;
...
/*지점 지도 뷰*/
CREATE VIEW STORE_MAP_VIEW AS
SELECT A.*,B.BRAND_NAME FROM TB_STORE A
JOIN TB_BRAND B ON(A.BRAND_NO = B.BRAND_NO)
